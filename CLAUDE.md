...
| 12a | **Computed Attributes** (collection aggregates via `GET /{resource}/computed?attributes=`, opt-in per-row values via `?computed_attributes=` on index/show/trashed; declared in `rhinoCollectionComputedAttributes()` / `rhinoRecordComputedAttributes()`. Attributes may declare PARAMETERS — `['params' => [...], 'optional' => [...], 'using' => fn]` — supplied as `?attributes[name][param]=value`, bound by name in declared order after `$user`; a bare `/computed` skips required-parameter attributes rather than 403ing. Merged before the policy blacklist/whitelist; undeclared and policy-denied return the same 403) | `GlobalController.php` (`computed`, `resolveRequestedCollectionAttributes`, `resolveRequestedComputedAttributes`), `Support/ComputedAttributeSpec.php`, `Support/ArgumentBinder.php`, `Traits/HidableColumns.php` |
